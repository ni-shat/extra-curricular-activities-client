...
const TeacherSlider = () => {
  ...
  useEffect(() => {
    ...
    const splitIndex = getPopularTeachers.length / 2; 
    // console.log(splitIndex)

    // Split the array into two halves
    const first = getPopularTeachers.slice(0, splitIndex);
    setFirstHalf(first);
    const second = getPopularTeachers.slice(splitIndex);
  ...
  return (
    <div >
      ...
      >
        {
          getPopularTeachers?.map((teacher) => (
            <>
              <SwiperSlide key={teacher._id} className={`relative border border-transparent`}>
